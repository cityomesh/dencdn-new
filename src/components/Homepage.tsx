...
const Homepage = () => {
  ...
  const [selectedSourceServer, setSelectedSourceServer] = useState<string>("all");
  ...
  // Get unique source servers from assignments for dropdown
  const uniqueSourceServers = useMemo(() => {
    const serversSet = new Set<string>();
    assignments.forEach(a => {
      if (a.sourceServer) {
        serversSet.add(a.sourceServer);
      }
    });
    return Array.from(serversSet).sort();
  }, [assignments]);
  ...
  useEffect(() => {
    ...
    if (selectedSourceServer !== "all") {
      filtered = filtered.filter(item => item.sourceServer === selectedSourceServer);
    }

    setFilteredAssignments(filtered);
    setCurrentPage(1);
    setSelectedRouteIds([]);
  }, [assignments, search, stream, selectedSourceServer]);
      ...
      <div className="px-20 py-28">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap items-center gap-4 mb-4">
            ...
            <select
              ...
              className="px-4 py-2 rounded bg-[#1a1a1a] border border-gray-700 text-sm min-w-[180px]"
            >
              <option value="all">All Source Servers</option>
              {uniqueSourceServers.map((serverName) => (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            {selectedSourceServer !== "all" && (
              <div className="px-3 py-1 rounded bg-orange-900/30 text-orange-300 text-sm">
