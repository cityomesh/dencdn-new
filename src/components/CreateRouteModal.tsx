...
const CreateRouteModal: React.FC<CreateRouteModalProps> = ({
  ...
  const handleSubmit = async () => {
    if (originUrls.length === 0 || selectedServers.length === 0) return;
    
    setIsSubmitting(true);
    setError(null);
    
    try {
      // Call the onSubmit prop which should now be async
      await onSubmit({
        originUrls,
        selectedServers
      });
      ...
      alert("✅ Route creation started!\n\nThis may take 30-60 seconds to complete on all servers.\nThe route will be active once the process finishes.");
      ...
      <div className="bg-[#1e1e1e] rounded-xl w-full max-w-5xl border border-gray-700 shadow-2xl overflow-hidden">
        ...
        <div className="p-8 space-y-10">
          ...
          <div className="space-y-4">
            ...
            {/* Tags Input Area */}
            <div className="space-y-3">
              <div className={`flex flex-wrap items-center gap-2 p-3 bg-[#252525] border rounded-lg transition-all duration-200 ${
                isSubmitting ? 'border-gray-700 opacity-50' : 'border-gray-700 focus-within:border-blue-500/70 focus-within:bg-[#262626]'
              }`}>
                ...
                {originUrls.map((url, index) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => removeUrl(index)}
                      disabled={isSubmitting}
                      className="p-0.5 hover:bg-blue-500/50 rounded-sm transition-colors disabled:opacity-50"
                      aria-label="Remove URL"
                    >
                      <FiX className="text-blue-300 text-xs" />
                    </button>
                ...
                <input
                  ...
                  disabled={isSubmitting}
                  placeholder={originUrls.length === 0 ? "Enter URL and press Enter or comma..." : "Add another URL..."}
                  className="flex-1 min-w-[240px] bg-transparent border-none outline-none text-white placeholder-gray-600 py-1.5 text-sm disabled:opacity-50"
                  autoFocus
                />
              </div>
          ...
          <div className="space-y-4">
            ...
            {availableServers.length === 0 ? (
              ...
              <div className="space-y-4">
                {/* Choose servers button with tags */}
                <div className={`flex flex-wrap items-center gap-2 p-3 bg-[#252525] border rounded-lg transition-all duration-200 ${
                  isSubmitting ? 'border-gray-700 opacity-50' : 'border-gray-700 focus-within:border-green-500/70'
                }`}>
                  ...
                  {selectedServers.map((server) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() => removeServer(server.id)}
                        disabled={isSubmitting}
                        className="p-0.5 hover:bg-green-500/50 rounded-sm transition-colors ml-0.5 disabled:opacity-50"
                        aria-label="Remove server"
                      >
                        <FiX className="text-green-300 text-xs" />
                      </button>
                ...
                {/* Server selection dropdown */}
                {showServerSelector && !isSubmitting && (
                  <div className="mt-3 p-1 bg-[#252525] rounded-lg border border-gray-700/80 shadow-xl max-h-64 overflow-y-auto">
                    <div className="sticky top-0 bg-[#252525] px-3 py-2 border-b border-gray-700/80 flex items-center justify-between">
                      <span className="text-xs font-medium text-gray-400">Available servers ({availableServers.length})</span>
                      <button
        ...
        <div className="flex items-center justify-end gap-3 px-8 py-5 bg-[#1a1a1a] border-t border-gray-700/80">
          <button
            onClick={onClose}
            disabled={isSubmitting}
            className="px-5 py-2.5 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm font-medium text-gray-300 transition-colors border border-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            disabled={originUrls.length === 0 || selectedServers.length === 0 || isSubmitting}
            className={`px-6 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 flex items-center gap-2 ${
              originUrls.length > 0 && selectedServers.length > 0 && !isSubmitting
                ? "bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white shadow-lg shadow-blue-600/20 border border-blue-500/50"
                : "bg-gray-800 text-gray-500 cursor-not-allowed border border-gray-700"
            }`}
          >
